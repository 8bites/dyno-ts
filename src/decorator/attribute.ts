...
export function Attribute(
    options: { name?: string; timeToLive?: true; default?: any } = {}
) {
    return (record: Table, propertyName: string) => {
        const tableClass = record.constructor as MetaTable<any>;
        const nativeType = Reflect.getMetadata(
            "design:type",
            record,
            propertyName
        );
        ...
        tableClass.metadata.attributes.push({
            ...
            type: nativeTypeToAttributeMetadataType(nativeType)
        });
    };
}

export function nativeTypeToAttributeMetadataType(nativeType: any) {
